# lean4-math-in-lean

Based on Mathematics in Lean by leanprover-community.

这是我学习 Lean 4 和 *Mathematics in Lean* 的公开学习仓库。目标是从 Chapter 1 重新开始，用小步提交记录自己的学习轨迹，并为 "AI4Math: Lean4 与数学形式化" 暑期学校申请准备可展示的 GitHub 记录。

## Repository Layout

- `MathProofs/`: 独立练习、试验和 proof scratchpad
- `MIL/`: 按章节整理的个人学习记录，只放我真正做过、改过、能解释的内容
- `notes/`: 可选的章节笔记、反思和踩坑记录
- `mathematics_in_lean/`: 本地参考教材仓库，保留在电脑上，但不纳入这个公开仓库

## Study Rules

- 公开仓库不镜像整套教材。
- 只提交我自己完成、整理或能够解释的内容。
- 学习时可以参考本地 `mathematics_in_lean/`，公开时只整理结果到这个仓库。
- 尽量保持小步提交，让 GitHub 能清楚展示学习过程。

## Current Progress

- [ ] C01 Introduction
- [ ] C02 Basics
- [ ] C03 Logic
- [ ] C04 Sets and Functions
- [ ] C05 Elementary Number Theory
- [ ] C06 Discrete Mathematics
- [ ] C07 Structures
- [ ] C08 Hierarchies
- [ ] C09 Groups and Rings
- [ ] C10 Linear Algebra
- [ ] C11 Topology
- [ ] C12 Differential Calculus
- [ ] C13 Integration and Measure Theory

## Workflow

1. 在本地 `mathematics_in_lean/` 里对照教材学习。
2. 把完成后的结果整理到 `MIL/` 或 `MathProofs/`。
3. 在 VS Code 的源代码管理面板里暂存、提交、推送。
4. 持续更新本 README 和章节进度。

## Upstream Reference

- Upstream textbook repo: <https://github.com/leanprover-community/mathematics_in_lean>
- Lean toolchain for this repo: see `lean-toolchain`