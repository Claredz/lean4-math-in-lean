...
theorem add_left_cancel {a b c : R} (h : a + b = a + c) : b = c := by
  rw [← add_zero b, ← add_neg_cancel a, ← add_assoc, add_comm b a, h,add_assoc,add_comm, add_assoc]
...
theorem mul_one (a : G) : a * 1 = a := by
  ...
  rw [mul_assoc,inv_mul_cancel] at h
  exact h

theorem mul_inv_rev (a b : G) : (a * b)⁻¹ = b⁻¹ * a⁻¹ := by
  have h : (b⁻¹ * a⁻¹) *( a * b) =1 := by
    rw [mul_assoc b⁻¹,← mul_assoc a⁻¹ ,inv_mul_cancel,one_mul,inv_mul_cancel]
  symm at h
  rw [← one_mul (a * b)⁻¹,h]
  rw [mul_assoc (b⁻¹ * a⁻¹)]
  rw [mul_inv_cancel]
